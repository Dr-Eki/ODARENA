    ...
    <div class="col-sm-12 col-md-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Welcome{{ $dominions->isEmpty() ? ' ' : ' back' }}, <strong>{{ Auth::user()->display_name }}</strong>!</h3>
            </div>
            <div class="box-body">
                <p>You have been playing since {{ $user->created_at }} and have participated in {{ number_format($dominions->count()) }} of {{ number_format($rounds->where('end_date', '>=', $user->created_at)->count()) }} completed rounds since joining.</p>
                <p><a href="{{ route('chronicles.ruler', $user->display_name) }}">View the Chronicles of {{ $user->display_name }}</a> for your full stats.</p>
            </div>
        </div>
    </div>
</div>

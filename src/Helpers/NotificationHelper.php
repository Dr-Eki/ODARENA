    ...
    public function getNotificationMessage(string $category, string $type, array $data, Dominion $dominion = null): string
    {
        switch ("{$category}.{$type}") {
            ...
            case 'hourly_dominion.summoning_completed':
                $units = array_sum($data);
                $term = $this->raceHelper->getSummoningTermVerb($dominion->race);
            ...
            case 'hourly_dominion.evolution_completed':
                $units = array_sum($data);
                $term = $this->raceHelper->getEvolutionTermVerb($dominion->race);
            ...
            case 'hourly_dominion.trade_failed_and_cancelled':
                return sprintf('You do not have enough %s for your trade route with %s. The trade route has been cancelled!',
                    $data['sold_resource_name'],
                    $data['hold_name']
                );
            ...
            case 'hourly_dominion.attrition_occurred':
                ...
                #$units = is_array($data) ? array_sum($data) : (float)$data;
                $term = $this->raceHelper->getAttritionTermVerb($dominion->race);
                return sprintf(
                    '%s %s %s',
                    is_numeric($unitCount) ? number_format($unitCount) : $unitCount,
                    is_numeric($unitCount) ? Str::plural('unit', $unitCount) : 'units',
                    $term
                );
